...
def astar(start, target, ogm, allow_goals=True):
    ...
    if not (0 <= tx < WAREHOUSE_WIDTH and 0 <= ty < WAREHOUSE_HEIGHT):
        return None
    
    # Check if target is an obstacle
    if ogm.is_obstacle(tx, ty):
        return None
    ...
    while open_set:
        # Sort by f-score and pop the best node
        ...
        if (cx, cy) == (tx, ty):
            ...
            path.reverse()
            debug_print(f"A* path found: {len(path)} steps from ({sx}, {sy}) to ({tx}, {ty})")
            return path
        ...
        for nx, ny in neighbors4(cx, cy):
            ...
            if ogm.is_obstacle(nx, ny):
                continue
            
            # Check if neighbor is FREE or GOAL (if goals are allowed)
            cell_state = ogm.get_cell_state(nx, ny)
            if cell_state == OCCUPIED:
                continue
            if not allow_goals and cell_state == GOAL:
                continue
            ...
            if (nx, ny) not in g_score or tentative_g < g_score[(nx, ny)]:
                ...
                if not any(nx == x and ny == y for _, x, y in open_set):
                    open_set.append((f, nx, ny))
    
    debug_print(f"A* path not found from ({sx}, {sy}) to ({tx}, {ty})")
