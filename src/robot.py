...
class Robot:
    ...
    def can_move_to(self, new_x, new_y, warehouse):
        ...
        # Also check OGM if it exists (for safety)
        if self.ogm and self.ogm.is_obstacle(int(new_x), int(new_y)):
            return False
        
        # Check path is clear using LIDAR
    ...
    def explore_next_cell(self, current_time):
        ...
        for dx, dy in directions:
            ...
            if 0 <= next_x < WAREHOUSE_WIDTH and 0 <= next_y < WAREHOUSE_HEIGHT:
                if self.warehouse.is_blocked(next_x, next_y) or self.ogm.is_obstacle(next_x, next_y):
        ...
        # If queue is empty, find nearest unvisited cell and path to it
        if not self.exploration_queue:
            ...
            if nearest:
                # Find path to nearest cell using simple BFS pathfinding
                ...
                if path:
                    ...
                    debug_log(f"Queue empty - found path to nearest unvisited cell {nearest} (distance: {min_dist})")
                else:
                    # Can't reach - try adjacent unvisited cells instead
                    for cell in unvisited:
                        ux, uy = cell
                        if self.warehouse.is_blocked(ux, uy) or self.ogm.is_obstacle(ux, uy):
                            continue
            ...
            # If still empty after all attempts, return False to let main loop handle completion check
            # The main loop (line 582) will properly check for unvisited cells
            if not self.exploration_queue:
                return False  # Wait for next frame, main loop will check completion properly
        
        # Get next cell from queue (skip obstacles and visited)
        attempts = 0
        max_attempts = len(self.exploration_queue)  # Limit attempts to queue size
        while self.exploration_queue and attempts < max_attempts:
            ...
            if not self.can_move_to(nx, ny, self.warehouse):
                # This cell can't be reached - mark it as obstacle in OGM if it's not already
        ...
        # Queue exhausted or all cells failed - find new unvisited cells immediately
        if not self.exploration_queue:
            total_free = self.warehouse.get_free_cells()
            unvisited = [cell for cell in total_free if cell not in self.visited]
            ...
            for nearest in sorted_unvisited[:20]:  # Try up to 20 nearest cells
                ...
                path = self._find_path_to_cell(current_pos, nearest)
                if not path:
                    # No path found - mark as obstacle
                    if not self.warehouse.is_blocked(ux, uy) and self.ogm:
                        self.ogm.mark_obstacle(ux, uy)
                    continue
                ...
                for cell in path[1:]:  # Skip first cell (current position)
                    ...
                    if not self.can_move_to(cell[0], cell[1], self.warehouse):
                        # Cell unreachable, mark as obstacle
                        if not self.warehouse.is_blocked(cell[0], cell[1]) and self.ogm:
                            self.ogm.mark_obstacle(cell[0], cell[1])
                        break
